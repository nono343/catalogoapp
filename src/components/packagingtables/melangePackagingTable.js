...
import MelangeDataPackaging from './datatable/melangePackagingTable';

function MelangePackagingTable({ isSpanish }) {
    const [products, setProducts] = useState([]);
    const [sortOrder, setSortOrder] = useState('asc');
    const [searchTerm, setSearchTerm] = useState('');
    const [melangeData, setMelangeData] = useState(null);
    ...
    useEffect(() => {
        ...
        const MelangeProductData = MelangeDataPackaging(isSpanish).Packaging;
        setMelangeData(MelangeProductData);
    }, [isSpanish]);
    
    useEffect(() => {
        // Este efecto actualiza la lista de productos cuando cambia melangeData o isSpanish
        if (melangeData && typeof melangeData === 'object') {
            const dataArray = Object.keys(melangeData).map(key => ({
                key: key,
                value: melangeData[key]
            }));
            setProducts(dataArray);
        }
    }, [melangeData, isSpanish]);
