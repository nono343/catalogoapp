...
import MycubiesDataPackaging from './datatable/mycubiesDataPackaging';

function MycubiesPackagingTable({ isSpanish }) {
    const [products, setProducts] = useState([]);
    const [sortOrder, setSortOrder] = useState('asc');
    const [searchTerm, setSearchTerm] = useState('');
    const [mycubiesData, setMycubiesData] = useState(null);
    ...
    useEffect(() => {
        ...
        const mycubiesProductData = MycubiesDataPackaging(isSpanish).Packaging;
        setMycubiesData(mycubiesProductData);
    }, [isSpanish]);
    
    useEffect(() => {
        // Este efecto actualiza la lista de productos cuando cambia mycubiesData o isSpanish
        if (mycubiesData && typeof mycubiesData === 'object') {
            const dataArray = Object.keys(mycubiesData).map(key => ({
                key: key,
                value: mycubiesData[key]
            }));
            setProducts(dataArray);
        }
    }, [mycubiesData, isSpanish]);
    ...
    return (
        <section className="body-font mb-5 max-w-screen-2xl mx-auto">
            ...
            <div className="relative overflow-x-auto px-12 shadow-md rounded-lg max-h-80 animate-fade-up">
...
export default MycubiesPackagingTable
