...
import SweetpalermoDataPackaging from './datatable/sweetpalermoDataPackaging';


function SweetpalermoPackagingTable({ isSpanish }) {
    const [products, setProducts] = useState([]);
    const [sortOrder, setSortOrder] = useState('asc');
    const [searchTerm, setSearchTerm] = useState('');
    const [sweetpalermoData, setSweetpalermoData] = useState(null);
    ...
    useEffect(() => {
        ...
        const sweetpalermoProductData = SweetpalermoDataPackaging(isSpanish).Packaging;
        setSweetpalermoData(sweetpalermoProductData);
    }, [isSpanish]);
    
    useEffect(() => {
        // Este efecto actualiza la lista de productos cuando cambia sweetpalermoData o isSpanish
        if (sweetpalermoData && typeof sweetpalermoData === 'object') {
            const dataArray = Object.keys(sweetpalermoData).map(key => ({
                key: key,
                value: sweetpalermoData[key]
            }));
            setProducts(dataArray);
        }
    }, [sweetpalermoData, isSpanish]);
...
export default SweetpalermoPackagingTable
