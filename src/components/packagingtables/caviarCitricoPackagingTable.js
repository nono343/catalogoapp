...
import CaviarcitricoDataPackaging from './datatable/caviarcitricoDataPackaging';


function CaviarcitricoPackagingTable({ isSpanish }) {
    const [products, setProducts] = useState([]);
    const [sortOrder, setSortOrder] = useState('asc');
    const [searchTerm, setSearchTerm] = useState('');
    const [caviarcitricoData, setCaviarcitricoData] = useState(null);
    ...
    useEffect(() => {
        ...
        const caviarcitricoProductData = CaviarcitricoDataPackaging(isSpanish).Packaging;
        setCaviarcitricoData(caviarcitricoProductData);
    }, [isSpanish]);
    
    useEffect(() => {
        // Este efecto actualiza la lista de productos cuando cambia caviarcitricoData o isSpanish
        if (caviarcitricoData && typeof caviarcitricoData === 'object') {
            const dataArray = Object.keys(caviarcitricoData).map(key => ({
                key: key,
                value: caviarcitricoData[key]
            }));
            setProducts(dataArray);
        }
    }, [caviarcitricoData, isSpanish]);
