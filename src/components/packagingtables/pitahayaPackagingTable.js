...
import PitahayaDataPackaging from './datatable/pitahayaDataPackaging';


function PitahayaPackagingTable({ isSpanish }) {
    const [products, setProducts] = useState([]);
    const [sortOrder, setSortOrder] = useState('asc');
    const [searchTerm, setSearchTerm] = useState('');
    const [pitahayaData, setPitahayaData] = useState(null);
    ...
    useEffect(() => {
        ...
        const pitahayaProductData = PitahayaDataPackaging(isSpanish).Packaging;
        setPitahayaData(pitahayaProductData);
    }, [isSpanish]);
    
    useEffect(() => {
        // Este efecto actualiza la lista de productos cuando cambia pitahayaData o isSpanish
        if (pitahayaData && typeof pitahayaData === 'object') {
            const dataArray = Object.keys(pitahayaData).map(key => ({
                key: key,
                value: pitahayaData[key]
            }));
            setProducts(dataArray);
        }
    }, [pitahayaData, isSpanish]);
