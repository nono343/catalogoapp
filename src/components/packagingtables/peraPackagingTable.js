...
import PeraDataPackaging from './datatable/peraDataPackaging';

function PeraPackagingTable({ isSpanish }) {
    const [products, setProducts] = useState([]);
    const [sortOrder, setSortOrder] = useState('asc');
    const [searchTerm, setSearchTerm] = useState('');
    const [peraData, setPeraData] = useState(null);
    ...
    useEffect(() => {
        ...
        const PeraProductData = PeraDataPackaging(isSpanish).Packaging;
        setPeraData(PeraProductData);
    }, [isSpanish]);
    
    useEffect(() => {
        // Este efecto actualiza la lista de productos cuando cambia peraData o isSpanish
        if (peraData && typeof peraData === 'object') {
            const dataArray = Object.keys(peraData).map(key => ({
                key: key,
                value: peraData[key]
            }));
...
export default PeraPackagingTable
