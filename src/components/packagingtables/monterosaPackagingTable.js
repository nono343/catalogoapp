...
import MonterosaDataPackaging from './datatable/monterosaDataPackaging';

function MonterosaPackagingTable({ isSpanish }) {
    const [products, setProducts] = useState([]);
    const [sortOrder, setSortOrder] = useState('asc');
    const [searchTerm, setSearchTerm] = useState('');
    const [monterosaData, setMonterosaData] = useState(null);
    ...
    useEffect(() => {
        ...
        const monterosaProductData = MonterosaDataPackaging(isSpanish).Packaging;
        setMonterosaData(monterosaProductData);
    }, [isSpanish]);
    
    useEffect(() => {
        // Este efecto actualiza la lista de productos cuando cambia monterosaData o isSpanish
        if (monterosaData && typeof monterosaData === 'object') {
            const dataArray = Object.keys(monterosaData).map(key => ({
                key: key,
                value: monterosaData[key]
            }));
            setProducts(dataArray);
        }
    }, [monterosaData, isSpanish]);
...
export default MonterosaPackagingTable
