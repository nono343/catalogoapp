...
import PrimoraDataPackaging from './datatable/primoraDataPackaging';

function PrimoraPackagingTable({ isSpanish }) {
    const [products, setProducts] = useState([]);
    const [sortOrder, setSortOrder] = useState('asc');
    const [searchTerm, setSearchTerm] = useState('');
    const [primoraData, setPrimoraData] = useState(null);
    ...
    useEffect(() => {
        ...
        const primoraProductData = PrimoraDataPackaging(isSpanish).Packaging;
        setPrimoraData(primoraProductData);
    }, [isSpanish]);
    
    useEffect(() => {
        // Este efecto actualiza la lista de productos cuando cambia primoraData o isSpanish
        if (primoraData && typeof primoraData === 'object') {
            const dataArray = Object.keys(primoraData).map(key => ({
                key: key,
                value: primoraData[key]
            }));
            setProducts(dataArray);
        }
    }, [primoraData, isSpanish]);
...
export default PrimoraPackagingTable
