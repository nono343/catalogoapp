...
import BerenjenajaponesaDataPackaging from './datatable/berenjenajaponesaDataPackaging';

function BerenjenajaponesaPackagingTable({ isSpanish }) {
    const [products, setProducts] = useState([]);
    const [sortOrder, setSortOrder] = useState('asc');
    const [searchTerm, setSearchTerm] = useState('');
    const [berenjenajaponesaData, setBerenjenajaponesaData] = useState(null);
    ...
    useEffect(() => {
        ...
        const berenjenajaponesaProductData = BerenjenajaponesaDataPackaging(isSpanish).Packaging;
        setBerenjenajaponesaData(berenjenajaponesaProductData);
    }, [isSpanish]);
    
    useEffect(() => {
        // Este efecto actualiza la lista de productos cuando cambia berenjenajaponesaData o isSpanish
        if (berenjenajaponesaData && typeof berenjenajaponesaData === 'object') {
            const dataArray = Object.keys(berenjenajaponesaData).map(key => ({
                key: key,
                value: berenjenajaponesaData[key]
            }));
            setProducts(dataArray);
        }
    }, [berenjenajaponesaData, isSpanish]);
...
export default BerenjenajaponesaPackagingTable
