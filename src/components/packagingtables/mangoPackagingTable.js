...
import MangoDataPackaging from './datatable/mangoDataPackaging';


function MangoPackagingTable({ isSpanish }) {
    const [products, setProducts] = useState([]);
    const [sortOrder, setSortOrder] = useState('asc');
    const [searchTerm, setSearchTerm] = useState('');
    const [mangoData, setMangoData] = useState(null);
    ...
    useEffect(() => {
        ...
        const mangoProductData = MangoDataPackaging(isSpanish).Packaging;
        setMangoData(mangoProductData);
    }, [isSpanish]);
    
    useEffect(() => {
        // Este efecto actualiza la lista de productos cuando cambia mangoData o isSpanish
        if (mangoData && typeof mangoData === 'object') {
            const dataArray = Object.keys(mangoData).map(key => ({
                key: key,
                value: mangoData[key]
            }));
...
export default MangoPackagingTable
