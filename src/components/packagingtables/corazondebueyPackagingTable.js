...
import CorazondebueyDataPackaging from './datatable/corazondebueyDataPackaging';

function CorazondebueyPackagingTable({ isSpanish }) {
    const [products, setProducts] = useState([]);
    const [sortOrder, setSortOrder] = useState('asc');
    const [searchTerm, setSearchTerm] = useState('');
    const [corazondebueyData, setCorazondebueyData] = useState(null);
    ...
    useEffect(() => {
        ...
        const CorazondebueyProductData = CorazondebueyDataPackaging(isSpanish).Packaging;
        setCorazondebueyData(CorazondebueyProductData);
    }, [isSpanish]);
    
    useEffect(() => {
        // Este efecto actualiza la lista de productos cuando cambia corazondebueyData o isSpanish
        if (corazondebueyData && typeof corazondebueyData === 'object') {
            const dataArray = Object.keys(corazondebueyData).map(key => ({
                key: key,
                value: corazondebueyData[key]
            }));
            setProducts(dataArray);
        }
    }, [corazondebueyData, isSpanish]);
