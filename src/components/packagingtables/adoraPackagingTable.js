...
import AdoraaDataPackaging from './datatable/adoraDataPackaging';

function AdoraPackagingTable({ isSpanish }) {
    const [products, setProducts] = useState([]);
    const [sortOrder, setSortOrder] = useState('asc');
    const [searchTerm, setSearchTerm] = useState('');
    const [adoraData, setAdoraData] = useState(null);
    ...
    useEffect(() => {
        ...
        const adoraProductData = AdoraaDataPackaging(isSpanish).Packaging;
        setAdoraData(adoraProductData);
    }, [isSpanish]);

    useEffect(() => {
        // Este efecto actualiza la lista de productos cuando cambia adoraData o isSpanish
        if (adoraData && typeof adoraData === 'object') {
            const dataArray = Object.keys(adoraData).map(key => ({
                key: key,
                value: adoraData[key]
            }));
