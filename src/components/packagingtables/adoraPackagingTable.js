...
function AdoraPackagingTable({ isSpanish }) {
    const TABLE_HEAD = isSpanish
        ...
        : ["Packaging", "Presentación (unidades/caja)", "Peso de Presentación (g)", "Peso Neto (kg)", "80x120 (cajas/palet)", "Peso Neto de 80x120 (kg)", "100x120 (cajas/palet)", "Imagen"];
    ...
    const [TABLE_ROWS, setTableRows] = useState([
        {
            ...
            weight80120: isSpanish ? "100" : "80x120 Net Weight (kg)",
        ...
        },
        {
            ...
            netWeight: isSpanish ? "Peso Neto (kg)" : "Net Weight (kg)",
            size80120: isSpanish ? "80x120 (cajas/palet)" : "80x120 (boxes/palet)",
            weight80120: isSpanish ? "50" : "80x120 Net Weight (kg)",
            size100120: isSpanish ? "100x120 (cajas/palet)" : "100x120 (boxes/palet)",
            imageSmall: amela1, // Imagen pequeña en la tabla
            imageLarge: amela1, // URL de la imagen grande
            isEnlarged: false, // Estado de ampliación inicialmente en falso
    ...
    const handleSort = (column) => {
        const sortedRows = TABLE_ROWS.slice(); // Copia los datos actuales

        sortedRows.sort((a, b) => {
            if (sortOrder === 'asc') {
                return parseFloat(a[column].replace(',', '.').trim()) - parseFloat(b[column].replace(',', '.').trim());
            } else {
                return parseFloat(b[column].replace(',', '.').trim()) - parseFloat(a[column].replace(',', '.').trim());
            }
        });

        setTableRows(sortedRows); // Actualiza el estado con las filas ordenadas
        setSortOrder(sortOrder === 'asc' ? 'desc' : 'asc'); // Invierte el orden
    };
                        ...
                        <thead>
                            <tr>
                                {TABLE_HEAD.map((head, index) => (
                                    <th key={head} className="border-b border-blue-gray-100 bg-blue-gray-50 p-4" onClick={() => handleSort(head)}>
                                        <Typography
                                            variant="small"
                                            color="blue-gray"
