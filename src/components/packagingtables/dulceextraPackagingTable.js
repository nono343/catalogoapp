...
import DulcextraDataPackaging from './datatable/dulcextraDataPackaging';

function DulcextraPackagingTable({ isSpanish }) {
    const [products, setProducts] = useState([]);
    const [sortOrder, setSortOrder] = useState('asc');
    const [searchTerm, setSearchTerm] = useState('');
    const [dulcextraData, setDulcextraData] = useState(null);
    ...
    useEffect(() => {
        ...
        const dulcextraProductData = DulcextraDataPackaging(isSpanish).Packaging;
        setDulcextraData(dulcextraProductData);
    }, [isSpanish]);
    
    useEffect(() => {
        // Este efecto actualiza la lista de productos cuando cambia dulcextraData o isSpanish
        if (dulcextraData && typeof dulcextraData === 'object') {
            const dataArray = Object.keys(dulcextraData).map(key => ({
                key: key,
                value: dulcextraData[key]
            }));
            setProducts(dataArray);
        }
    }, [dulcextraData, isSpanish]);
...
export default DulcextraPackagingTable
