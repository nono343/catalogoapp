...
import PicantesDataPackaging from './datatable/picantesDataPackaging';

function PicantesPackagingTable({ isSpanish }) {
    const [products, setProducts] = useState([]);
    const [sortOrder, setSortOrder] = useState('asc');
    const [searchTerm, setSearchTerm] = useState('');
    const [picantesData, setPicantesData] = useState(null);
    ...
    useEffect(() => {
        ...
        const picantesProductData = PicantesDataPackaging(isSpanish).Packaging;
        setPicantesData(picantesProductData);
    }, [isSpanish]);
    
    useEffect(() => {
        // Este efecto actualiza la lista de productos cuando cambia picantesData o isSpanish
        if (picantesData && typeof picantesData === 'object') {
            const dataArray = Object.keys(picantesData).map(key => ({
                key: key,
                value: picantesData[key]
            }));
            setProducts(dataArray);
        }
    }, [picantesData, isSpanish]);
