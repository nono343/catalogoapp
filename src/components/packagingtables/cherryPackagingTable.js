...
import CherryDataPackaging from './datatable/cherryDataPackaging';


function CherryPackagingTable({ isSpanish }) {
    const [products, setProducts] = useState([]);
    const [sortOrder, setSortOrder] = useState('asc');
    const [searchTerm, setSearchTerm] = useState('');
    const [cherryData, setCherryData] = useState(null);
    ...
    useEffect(() => {
        ...
        const cherryProductData = CherryDataPackaging(isSpanish).Packaging;
        setCherryData(cherryProductData);
    }, [isSpanish]);
    
    useEffect(() => {
        // Este efecto actualiza la lista de productos cuando cambia amelaData o isSpanish
        if (cherryData && typeof cherryData === 'object') {
            const dataArray = Object.keys(cherryData).map(key => ({
                key: key,
                value: cherryData[key]
            }));
            setProducts(dataArray);
        }
    }, [cherryData, isSpanish]);
